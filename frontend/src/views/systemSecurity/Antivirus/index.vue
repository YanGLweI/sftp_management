          ...
          <el-card shadow="never">
            ...
            >
              ...
              <el-form-item label="计划" prop="ruleType">
                <el-select v-model="form.ruleType" placeholder="请选择任务启动计划" @change="handleScanRuleTypeChange">
              ...
              <!-- 日期时间设置 -->
              <el-form-item label="日期" prop="date" v-if="form.ruleType === 'Once' || form.ruleType === 'Hourly'">
                <el-date-picker
                  v-model="form.date"
                  type="date"
        ...
        <el-tab-pane label="保护状态报告" name="reportPlan">
          <el-card shadow="never">
            ...
            >
              <!-- 扫描间隔设置 -->
              <el-form-item label="计划" prop="ruleType">
                <el-select v-model="reportForm.ruleType" placeholder="请选择任务启动计划" @change="handleReportRuleTypeChange">
                  <el-option label="每天" value="Daily"></el-option>
                  <el-option label="每周" value="Weekly"></el-option>
                </el-select>
              </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      reportRules: {
        ...
        interval: [
          { required: true, message: '请选择任务执行间隔', trigger: 'blur' }
        ],
        enable: [
          { required: true, message: '请选择是否启用计划任务', trigger: 'change' }
        ]
      },
      // 隔离区状态
      isolationZoneStatus: '',
  ...
  methods: {
    ...
    resetAllForm() {
      this.$nextTick(() => {
        // 重置扫描任务表单
        if (this.$refs.scanFormRef) {
          this.$refs.scanFormRef.resetFields();
    ...
    handleSubmit() {
      // 获取当前激活的表单Ref
      const formRef = this.activeName === 'scanPlan' 
        ? this.$refs.scanFormRef 
        : this.$refs.reportFormRef;

      if (!formRef) {
        this.$message.error("表单加载异常，请重试");
        return;
      }
      ...
      formRef.validate(async (valid) => {
        if (valid) {
          try {
            ...
            this.$message.error("计划设置失败，请稍后重试");
          } finally {
    ...
    */
    ...
    startScan() {
      this.$confirm("确定要执行立即扫描操作吗？", "提示", {
        ...
        try {
          ...
          this.$message.error("扫描启动失败，请稍后重试");
        } finally {
          this.getAntivirusInfo();
        }
      }).catch(() => {
    ...
    async getIsolationZone() {
      try {
        ...
        if (res.code === 200) {
          this.isolationZoneStatus = res.data === "" ? "正常" : "异常";
        } else {
          this.isolationZoneStatus = "异常";
        }
      } catch (e) {
        console.error('获取隔离区状态失败：', e);
        this.isolationZoneStatus = "异常";
      }
    },
